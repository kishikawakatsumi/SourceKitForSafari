...
final class SourceKitServiceProxy {
    ...
    func sendReferencesRequest(resource: String, slug: String, path: String, line: Int, character: Int, completion: @escaping (Bool, [String: Any]) -> Void) {
        let connection = self.connection
        let context = self.context
        ...
        queue.async {
            ...
            service.sendReferencesRequest(context: context, resource: resource, slug: slug, path: path, line: line, character: character) { (successfully, response) in
                completion(successfully, response)
            }
        }
    }
